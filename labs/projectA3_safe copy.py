...
for k in [1, 7]:
    ...
    for start_index, n in zip(start_indexes, n_weeks):
        test_model(df, k=k, start_index=start_index, n_weeks=n,
                   BJmodel=bjModel, buffer=200, plot=True)
...
def solutionA_start0(payload):
    ...
    # === BJ model (NEW) ===
    B = np.array([-0.5285, 0.2124])          # nB=1
    F = np.array([1.0, -0.2474])             # nF=1
